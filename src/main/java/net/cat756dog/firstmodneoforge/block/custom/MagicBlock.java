package net.cat756dog.firstmodneoforge.block.custom;

import net.cat756dog.firstmodneoforge.block.ModBlocks;
import net.cat756dog.firstmodneoforge.item.ModItems;
...
public class MagicBlock extends Block {
    ...
    public void stepOn(Level level, BlockPos pos, BlockState state, Entity entity) {
        ...
        if(entity instanceof ItemEntity itemEntity) {
            if (itemEntity.getItem().getItem() == ModBlocks.BISMUTH_DEEPSLATE_ORE.asItem()) {
                itemEntity.setItem(new ItemStack(ModItems.BISMUTH.get(), itemEntity.getItem().getCount()));
            }
        }
        if(entity instanceof ItemEntity itemEntity) {
            if (itemEntity.getItem().getItem() == ModBlocks.BISMUTH_ORE.asItem()) {
                itemEntity.setItem(new ItemStack(ModItems.BISMUTH.get(), itemEntity.getItem().getCount()));
            }
        }
        if(entity instanceof ItemEntity itemEntity) {
        ...
        if(entity instanceof ItemEntity itemEntity) {
            if (itemEntity.getItem().getItem() == Items.CHARCOAL) {
